...
public:  
    ...
    inline VariableHolder(): VariableHolder(0.) {
    } 
    
    inline VariableHolder(const std::string& name_, const BlockHolder& bh)
        : it(__variable_map.register_value(name_, Variable(name_, ""))) 
    {
        operator=(bh);
    }
    inline VariableHolder(const BlockHolder& bh)
        : it(__variable_map.emplace("", ""))
    {
        it->second.name = it->first; 
        operator=(bh);
    }
