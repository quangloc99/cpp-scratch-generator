...
public:
    ...
    static void pop_stack_til(const std::string& id) {
        while (true) {
            assert(!block_holder_stack.empty());
            if (block_holder_stack.back().id() == id) {
                break;
            }
...
public:
    ...
    ~IfBlockGenerator() {
        ...
        BlockHolder::pop_stack_til(if_block.id());
    }
...
class ElseBlockGenerator {
    bool collecting;
    BlockHolder if_block;
public:
    ElseBlockGenerator(): collecting(0) {
    ...
    ~ElseBlockGenerator() {
        if (!if_block->next.empty()) {
            if_block->inputs.emplace("SUBSTACK2", BlockInput::id(if_block->next));
            if_block->next = "";
        }
        BlockHolder::pop_stack_til(if_block.id());
    }
